...
from chemprop_ms.models import build_model, MoleculeModel
from chemprop_ms.models.prompt_generator import add_functional_prompt
from chemprop_ms.nn_utils import NoamLR
...
def load_args(path: str) -> Namespace:
    ...
    """
    return ms.load_checkpoint(path)['args']
...
def get_loss_func(args: Namespace) -> nn.Cell:
    """
...
def build_optimizer(model: nn.Cell, scheduler, args: Namespace):
    """
    Builds an Optimizer.
    ...
    no_increase_param = [param for name, param in model.parameters_and_names() if not any(inc in name for inc in increase)]
    increase_param = [param for name, param in model.parameters_and_names() if any(inc in name for inc in increase)]
    ...
    return nn.Adam(params, learning_rate=scheduler)
